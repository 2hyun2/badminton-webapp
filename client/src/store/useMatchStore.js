...
const useMatchStore = create((set, get) => ({
    ...
    startMatch: async () => {
        ...
        if (selectedPlayerIds.length < 4) return; // 매칭 리스트 인원수 4명 미만일시 false
        try {
            const response = await axios.post(`${HOST}/api/match/start`, {
                selectedIds: selectedPlayerIds
            }); // POST selectedPlayerIds[] 인원 
            if (response.status === 200) {
                set({ userList: response.data.updatedList, selectedPlayerIds: [] });
                return true;
            } // POST 결과 값 승인시 backend updateList *userList* 에 업데이트 && selectedPlayerIds 초기화
        } catch (error) {
