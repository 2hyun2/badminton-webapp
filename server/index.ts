...
import express, { Application } from 'express'; // 웹 서버 프레임워크
...
const httpSerer: Server = http.createServer(app);

// socket의 타입을 명시적으로 Socket으로 지정
io.on('connection', (socket: Socket) => {
    console.log('클라이언트 연결:', socket.id);

    // query에서 가져온 userId는 문자열 배열일 수도 있으므로, string으로 타입 단언(Type Assertion)
...
// 1. Mongoose Document 타입을 상속받는 인터페이스 정의
// User 모델의 데이터 타입
interface IUser extends Document {
    id: number;
    username: string;
    password?: string; // select: false 인 필드는 조회 시 없을 수 있으므로 선택적(?) 속성으로 지정
    birthday: string;
    name: string;
    gender: string;
    tier: string | null;
...
// 출석 스키마 리스트
const dailyRecordSchema = new mongoose.Schema<IDailyRecord>({
...
const Match: Model<IMatch> = mongoose.model<IMatch>("Match", matchSchema, "matches");
const DailyRecord: Model<IDailyRecord> = mongoose.model<IDailyRecord>("DailyRecord", dailyRecordSchema, "daily_records");
const Counter: Model<ICounter> = mongoose.model<ICounter>("Counter", counterSchema, "counters");
...
const userAuth = (req, res, next) => {
    ...
    try {
        // 2. 토큰 해독 및 검증 (.env의 JWT_SECRET 사용)
        const decoded = jwt.verify(token, process.env.JWT_SECRET);

        // 3. 해독된 정보(로그인할 때 넣은 userId, role 등)를 req.user에 저장
        req.user = decoded;

        // 4. 통과! 다음 본 API 로직으로 이동
        next();
    } catch (error) {
...
app.post('/api/users/update', userAuth, async (req, res) => {
    try {
        ...
        const bulkOps = await Promise.all(items
            ...
            .map(async (user) => {
                ...
                for (const key of ALLOWED_USER_FIELDS) {
                    // password는 빈 값일 때 수정을 막아야 하지만, bio 등 다른 필드는 빈 값 허용 필요
...
app.post("/api/match/end", async (req, res) => {
    ...
    try {
        ...
        // dailyRecord matches update
        if (winnerTeam !== 'cancel') {

            const dailyRecordOps = matchedUser.map(user => {
                const isTeamA = user.matchSlot === 0 || user.matchSlot === 1;
                const isTeamB = user.matchSlot === 2 || user.matchSlot === 3;
                const isWin = (winnerTeam === 'A' && isTeamA) || (winnerTeam === 'B' && isTeamB);
                const change = isTeamA ? ratingChange : -ratingChange;
                ...
                return {
                    updateOne: {
                        filter: { date: getToday(), userId: user.id },
                        update: {
                            $set: { endRating: user.rating + change },
                            $push: {
                                matches: {
                                    matchId: matchId,
                        ...
                        upsert: true // 당일 첫 경기일 경우 문서 자동 생성
                    }
                };
            });

            await DailyRecord.bulkWrite(dailyRecordOps);
        }
...
app.post('/api/admin/users/:userId/reset-status', adminOnly, async (req, res) => {
    try {
        ...
        const updatedUser = await User.findOneAndUpdate(
            { id: userId },
            { $set: { status: "RESTING", matchId: null, matchSlot: null, updatedAt: getKSTNow() } },
            { returnDocument: 'after' } // findOneAndUpdate 에 있는 기능
        );
        if (!updatedUser) return res.status(404).json({ message: '유저 없음' });
...
app.delete('/api/admin/matches/:matchId', adminOnly, async (req, res) => {
    try {
        ...
        const targetMatch = await Match.findOne({ matchId: Number(matchId) });

        if (!targetMatch) {
            return res.status(404).json({ message: '경기 기록을 찾을 수 없습니다.' });
        }

        // 1. 경기 삭제 전 DailyRecord 동기화 (승/패 카운트 차감 및 목록 제거)
        ...
        // 1. User 테이블 누적 통계 복구 (레이팅, 판수, 승패 롤백)
        ...
        // 2. DailyRecord 동기화 (endRating 보정 및 matches 배열에서 제거)
        ...
        // 4. 실제 Match 데이터 삭제
        const deletedMatch = await Match.findOneAndDelete({ matchId: Number(matchId) });

        if (!deletedMatch) return res.status(404).json({ message: '삭제할 경기를 찾지 못했습니다.' });

        io.emit('match:update', { type: 'DELETE', matchId: deletedMatch.matchId });
        io.emit('users:update', { type: 'UPDATE' }); // 일일 승패 수치 반영을 위해 유저 리스트 갱신 알림
